(() => {
  ...
  var require_bridge_entry = __commonJS({
    "../dip_client/MeetSpace/WebView/MediaHost/bridge-entry.js"() {
      ...
      var MICROPHONE_AUDIO_CONSTRAINTS_HD = {
      ...
      var SCREEN_VIDEO_CONSTRAINTS_BALANCED = {
        width: { ideal: 1280, max: 1280 },
        height: { ideal: 720, max: 720 },
        frameRate: { ideal: 30, max: 30 }
      };
      var SCREEN_VIDEO_CONSTRAINTS = SCREEN_VIDEO_CONSTRAINTS_HD;
      ...
      var SCREEN_FALLBACK_CONSTRAINTS = [
      ...
      var MICROPHONE_FALLBACK_CONSTRAINTS = [
      ...
      function buildTransportOptions(payload) {
        ...
        if (Array.isArray(payload.iceServers) && payload.iceServers.length > 0) {
          options.iceServers = payload.iceServers;
        }
        return options;
      }
        ...
        try {
          switch (command) {
            ...
            case "create_send_transport": {
              ...
              monitorIceState(state.sendTransport, "send");
              startQosCollection();
            ...
            case "create_recv_transport": {
              ...
              monitorIceState(state.recvTransport, "recv");
      ...
      var qosIntervalId = null;
      function startQosCollection() {
        if (qosIntervalId) return;
        qosIntervalId = setInterval(() => {
          void collectAndPostQos().catch(() => {
          });
        }, QOS_INTERVAL_MS);
      }
      function stopQosCollection() {
        if (qosIntervalId) {
          clearInterval(qosIntervalId);
          qosIntervalId = null;
        }
      }
      async function collectAndPostQos() {
        const tracks = [];
        await collectTransportQos(state.sendTransport, "send", tracks);
        await collectTransportQos(state.recvTransport, "recv", tracks);
        if (tracks.length > 0) {
          try {
            post({
              kind: "call_quality",
      ...
      function monitorIceState(transport, direction) {
        if (!transport) return;
        try {
      ...
      function computeBitrateKbps(direction, report) {
        if (!report || typeof report !== "object")
          return 0;
        const reportId = `${direction}:${report.id || report.ssrc || report.type || "unknown"}`;
        const timestampMs = toFiniteNumber(report.timestamp);
        const currentBytes = toFiniteNumber(report.bytesSent) + toFiniteNumber(report.bytesReceived);
        if (timestampMs <= 0 || currentBytes <= 0)
          return 0;
        const previous = state.qosPreviousByTrack.get(reportId);
        state.qosPreviousByTrack.set(reportId, {
          timestampMs,
          bytes: currentBytes
        });
        if (!previous || timestampMs <= previous.timestampMs || currentBytes < previous.bytes)
          return 0;
      ...
      function extractTrackQosFromStats(stats, direction, tracks) {
        ...
        stats.forEach((report) => {
          ...
          const roundTripTime = toFiniteNumber(
            report.roundTripTime ?? report.currentRoundTripTime ?? report.totalRoundTripTime
          );
          tracks.push({
            direction,
            kind: (report.kind || report.mediaType || "unknown").toString(),
            packetsSent: toFiniteNumber(report.packetsSent),
            packetsReceived: toFiniteNumber(report.packetsReceived),
            packetsLost: toFiniteNumber(report.packetsLost),
            jitter: toFiniteNumber(report.jitter),
            roundTripTime,
            bytesSent: toFiniteNumber(report.bytesSent),
            bytesReceived: toFiniteNumber(report.bytesReceived),
            bitrateKbps: computeBitrateKbps(direction, report)
          });
        });
      }
      async function collectTransportQos(transport, direction, tracks) {
        if (!transport || typeof transport.getStats !== "function")
          return;
        try {
          const stats = await transport.getStats();
          extractTrackQosFromStats(stats, direction, tracks);
        } catch (_) {
        }
      }
      registerBridge();
